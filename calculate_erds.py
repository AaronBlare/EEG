...
for electrode_id in range(0, num_electrodes):
    for movement_type in movement_types:
        Path(f"{figures_path}/{suffix}/{movement_type}/1D/").mkdir(parents=True, exist_ok=True)
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=np.linspace(0, min_trial_len / 1000, min_trial_len),
                                 y=ERDS_1D[movement_type][electrode_id],
                                 mode='lines'))
        ...
        fig.write_image(f"{figures_path}/{suffix}/{movement_type}/1D/{data_info['ch_names'][electrode_id]}.png")
        fig.write_image(f"{figures_path}/{suffix}/{movement_type}/1D/{data_info['ch_names'][electrode_id]}.pdf")
