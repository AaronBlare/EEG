...
def cwt(signal, dt, dj=1/12, s0=-1, J=-1, wavelet='morlet'):
    ...
    >> wave, scales, freqs, coi, fft, fftfreqs = wavelet.continuous_wavelet_transform(var,
    ...
    return (W[:, :n0], sj, freqs, coi, signal_ft[1:int(N/2)] / N ** 0.5,
            ftfreqs[1:int(N/2)] / (2 * np.pi))
...
def icwt(W, sj, dt, dj=1/12, wavelet='morlet'):
    ...
    ----------
    W : numpy.ndarray
        Wavelet transform, the result of the continuous_wavelet_transform function.
    sj : numpy.ndarray
        Vector of scale indices as returned by the continuous_wavelet_transform function.
    dt : float
        Sample spacing.
    dj : float, optional
        Spacing between discrete scales as used in the continuous_wavelet_transform
...
def significance(signal, dt, scales, sigma_test=0, alpha=None,
    ...
    scales : array like
        Vector of scale indices given returned by continuous_wavelet_transform function.
...
def wct(signal, signal2, dt, dj=1/12, s0=-1, J=-1, sig=True, significance_level=0.95,
    ...
    See also
    --------
    wavelet.continuous_wavelet_transform, wavelet.xwt
    """
    if isinstance(wavelet, str):
        wavelet = mothers[wavelet]()
